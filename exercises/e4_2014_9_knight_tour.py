...
def fast_tour(board, x, y) -> bool:
    ...
    while n < w * h:
        ...
        if len(moves) == 0:
            return False
        n += 1
##        x, y = min(moves, key=lambda m: border_distance(m[0], m[1], w, h))
        x, y = min(moves, key=lambda m: len(find_moves(board, m[0], m[1])))
        board[y][x] = n
    return True
...
def main():
    ...
    for s in range(5, 100):
        ...
        print(w, h, knight_tour(board, 0, 0))

        if max(w, h) <= 8:
            for y in range(h):
                for x in range(w):
                    print(("{:4}").format(board[y][x]), end='')
                print()
