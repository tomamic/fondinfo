...
class TicTacToe(BoardGame):
    ...
    def play(self, x: int, y: int, action: str):
        if self._get(x, y) == 0:
            self._board[y * self._l + x] = self._turn
            ...
            self._tie = not self._won and all(self._board)
    ...
    def status(self) -> str:
        tie, won, p = self._tie, self._won, "X" if self._turn == 1 else "O"
        return "Tie game" if tie else f"{p} has won" if won else f"{p} turn"
